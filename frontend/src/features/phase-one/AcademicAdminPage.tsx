...
import { createAcademic, listAcademic, deleteAcademic, type AcademicPath } from './api'
...
export function AcademicAdminPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: async () => { form.reset({ entity }); await client.invalidateQueries({ queryKey: ['academic'] }) },
  })
  ...
  return (
    <section className="page-stack">
      ...
      <form className="panel form-grid" onSubmit={form.handleSubmit((values) => create.mutate(values))}>
        ...
        {entity === 'grades' && <><label>Nivel<select {...form.register('level')}><option>inicial</option><option>primaria</option><option>secundaria</option></select></label><label>Orden<input type="number" min="1" {...form.register('order')} /></label><label>Periodo<select {...form.register('academic_period_id')}>{queries[0].data?.data.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label></>}
        {entity === 'sections' && <><label>Grado<select {...form.register('grade_id')}>{queries[1].data?.data.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Capacidad<input type="number" min="1" {...form.register('capacity')} required /></label></>}
        {entity === 'courses' && <label>Código<input {...form.register('code')} required /></label>}
        {entity === 'enrollments' && <><label>ID alumno<input {...form.register('student_id')} required /></label><label>Sección<select {...form.register('section_id')}>{queries[2].data?.data.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Periodo<select {...form.register('academic_period_id')}>{queries[0].data?.data.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label></>}
        {entity === 'teaching-assignments' && <><label>ID docente<input {...form.register('teacher_id')} required /></label><label>Curso<select {...form.register('course_id')}>{queries[3].data?.data.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Sección<select {...form.register('section_id')}>{queries[2].data?.data.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Periodo<select {...form.register('academic_period_id')}>{queries[0].data?.data.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label></>}
        <button className="button button-primary" disabled={create.isPending}>Crear</button>
